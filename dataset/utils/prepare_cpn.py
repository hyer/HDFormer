...
pred_path = join(base_path, 'data_cpn.npz')
...
pred = np.load(pred_path, allow_pickle=True)["positions_2d"].item()
...
for idx, k in enumerate(sorted(data_3d)):
    ...
    structured_2d_pred[k] = pred["S{}".format(k[0])][k[2].split(".")[0]][cam_id - 1][:data_3d[k].shape[0], :]  # [T,V,C]
...
np.savez_compressed(join(save_dir, 'cpn.npz'), data=structured_2d_pred, center_data_stat=center_data_stat,
